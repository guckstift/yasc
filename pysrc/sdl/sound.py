...
_dll = dll.SDL_DLL('SDL_sound', None)
...
_dll._version = dll._version_parts(Sound_GetLinkedVersion())
...
class Sound_Sample(Structure):
    ...
    def __getattr__(self, name):
        if name == 'decoder':
            ...
            return array.SDL_array(self._buffer, self.buffer_size, c_ubyte)
        raise AttributeError, name
...
Sound_NewSample = _dll.function('Sound_NewSample',
    ...
    arg_types=[POINTER(rwops.SDL_RWops), c_char_p,
...
def Sound_NewSampleFromMem(data, ext, desired, bufferSize):
    ...
    '''
    ref, data = array.to_ctypes(data, len(data), c_ubyte)

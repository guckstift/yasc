...
class SDLAppDelegate(NSObject):
    def applicationShouldTerminate_(self, app):
        ...
        event.type = SDL_QUIT
        events.SDL_PushEvent(event)
        return NSTerminateLater

    def windowUpdateNotification_(self, notification):
        win = notification.object()
        if not dll.version_compatible((1, 2, 8)) and isinstance(win, objc.lookUpClass('SDL_QuartzWindow')):
            # Seems to be a retain count bug in . workaround!
            win.retain()

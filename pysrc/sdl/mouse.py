...
class SDL_Cursor(Structure):
    ...
    def __getattr__(self, name):
        ...
        if name == 'data':
            return array.SDL_array(self._data, w * h / 8, c_ubyte)
        elif name == 'mask':
            return array.SDL_array(self._mask, w * h / 8, c_ubyte)
        raise AttributeError, name

_SDL_GetMouseState = dll.private_function('SDL_GetMouseState',
...
_SDL_GetRelativeMouseState = \
    dll.private_function('SDL_GetRelativeMouseState',
    arg_types=[POINTER(c_int), POINTER(c_int)],
    return_type=c_ubyte)
...
SDL_WarpMouse = dll.function('SDL_WarpMouse',
...
_SDL_CreateCursor = dll.private_function('SDL_CreateCursor',
...
def SDL_CreateCursor(data, mask, w, h, hot_x, hot_y):
    ...
    :rtype: `SDL_Cursor`
    '''
    dataref, data = array.to_ctypes(data, len(data), c_ubyte)
    maskref, mask = array.to_ctypes(mask, len(mask), c_ubyte)
    return _SDL_CreateCursor(data, mask, w, h, hot_x, hot_y)

SDL_SetCursor = dll.function('SDL_SetCursor',
...
SDL_GetCursor = dll.function('SDL_GetCursor',
...
SDL_FreeCursor = dll.function('SDL_FreeCursor',
...
SDL_ShowCursor = dll.function('SDL_ShowCursor',
...
SDL_BUTTON_LMASK    = SDL_BUTTON(constants.SDL_BUTTON_LEFT)
SDL_BUTTON_MMASK    = SDL_BUTTON(constants.SDL_BUTTON_MIDDLE)
SDL_BUTTON_RMASK    = SDL_BUTTON(constants.SDL_BUTTON_RIGHT)

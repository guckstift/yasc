...
class SDL_Palette(Structure):
    ...
    def __getattr__(self, name):
        if name == 'colors':
            return array.SDL_array(self._colors, self.ncolors, SDL_Color)
...
class SDL_Surface(Structure):
    ...
    def __getattr__(self, name):
        if name == 'format':
            ...
            if not self._pixels:
                raise error.SDL_Exception, 'Surface needs locking'
            ...
            if bpp == 1:
                ...
                raise error.SDL_Exception, 'Unsupported bytes-per-pixel'
            return array.SDL_array(self._pixels, count, sz)
        raise AttributeError, name
...
def SDL_MUSTLOCK(surface):
    ...
    return surface._offset or \
        ((surface.flags & \
          (constants.SDL_HWSURFACE | \
           constants.SDL_ASYNCBLIT | \
           constants.SDL_RLEACCEL)) != 0)
...
class SDL_VideoInfo(Structure):
    ...
    def __getattr__(self, name):
        ...
        if name in ('current_w', 'current_h'):
            dll.assert_version_compatible(name, (1,2,10))
...
class SDL_Overlay(Structure):
    ...
    def __getattr__(self, name):
        ...
        if name == 'hw_overlay':
            ...
            if not self._pixels:
                raise error.SDL_Exception, 'Overlay needs locking'
            p = []
            for i in range(self.planes):
                sz = self.pitches[i] * self.h
                p.append(array.SDL_array(self._pixels[i], sz, c_byte))
            return p
...
_SDL_VideoDriverName = dll.private_function('SDL_VideoDriverName',
...
SDL_GetVideoSurface = dll.function('SDL_GetVideoSurface',
...
SDL_GetVideoInfo = dll.function('SDL_GetVideoInfo',
...
SDL_VideoModeOK = dll.function('SDL_VideoModeOK',
...
_SDL_ListModes = dll.private_function('SDL_ListModes',
...
SDL_SetVideoMode = dll.function('SDL_SetVideoMode',
...
_SDL_UpdateRects = dll.private_function('SDL_UpdateRects',
...
def SDL_UpdateRects(screen, rects):
    ...
    '''
    ref, ar = array.to_ctypes(rects, len(rects), SDL_Rect)
    _SDL_UpdateRects(screen, len(rects), ar)

SDL_UpdateRect = dll.function('SDL_UpdateRect',
...
SDL_Flip = dll.function('SDL_Flip',
...
SDL_SetGamma = dll.function('SDL_SetGamma',
...
_SDL_SetGammaRamp = dll.private_function('SDL_SetGammaRamp',
...
def SDL_SetGammaRamp(red, green, blue):
    ...
    rar = gar = bar = None
    if red:
        rref, rar = array.to_ctypes(red, 256, c_ushort)
    if green:
        gref, gar = array.to_ctypes(green, 256, c_ushort)
    if blue:
        bref, bar = array.to_ctypes(blue, 256, c_ushort)
    result = _SDL_SetGammaRamp(rar, gar, bar)
    if result != 0:
        raise error.SDL_Exception, error.SDL_GetError()

_SDL_GetGammaRamp = dll.private_function('SDL_GetGammaRamp',
    arg_types=[POINTER(c_ushort), POINTER(c_ushort), POINTER(c_ushort)],
    return_type=c_int)
...
def SDL_GetGammaRamp():
    ...
    '''
    rar = array.SDL_array(None, 256, c_ushort)
    gar = array.SDL_array(None, 256, c_ushort)
    bar = array.SDL_array(None, 256, c_ushort)
    if _SDL_GetGammaRamp(rar.as_ctypes(), gar.as_ctypes(), bar.as_ctypes()) == 0:
        return rar, gar, bar
    return None, None, None

_SDL_SetColors = dll.private_function('SDL_SetColors',
...
def SDL_SetColors(surface, colors, firstcolor):
    ...
    '''
    ref, ar = array.to_ctypes(colors, len(colors), SDL_Color)
    return _SDL_SetColors(surface, ar, firstcolor, len(colors))

_SDL_SetPalette = dll.private_function('SDL_SetPalette',
...
SDL_MapRGB = dll.function('SDL_MapRGB',
...
SDL_MapRGBA = dll.function('SDL_MapRGBA',
...
_SDL_GetRGB = dll.private_function('SDL_GetRGB',
...
_SDL_GetRGBA = dll.private_function('SDL_GetRGBA',
    arg_types=[c_uint, POINTER(SDL_PixelFormat), 
...
SDL_CreateRGBSurface = dll.function('SDL_CreateRGBSurface',
...
_SDL_CreateRGBSurfaceFrom = \
    dll.private_function('SDL_CreateRGBSurfaceFrom',
...
def SDL_CreateRGBSurfaceFrom(pixels, width, height, depth, pitch,
    ...
    if hasattr(pixels, 'contents'):
        ...
        if len(pixels) == pitch * 8 / depth * height:
            # pixel array?
            if depth == 8:
                ref, ar = array.to_ctypes(pixels, len(pixels), c_ubyte)
            elif depth == 16:
                ref, ar = array.to_ctypes(pixels, len(pixels), c_ushort)
            elif depth == 32:
                ref, ar = array.to_ctypes(pixels, len(pixels), c_uint)
...
SDL_FreeSurface = dll.function('SDL_FreeSurface',
    '''Free an RGB Surface.
...
SDL_LockSurface = dll.function('SDL_LockSurface',
...
SDL_UnlockSurface = dll.function('SDL_UnlockSurface',
...
SDL_LoadBMP_RW = dll.function('SDL_LoadBMP_RW',
...
def SDL_LoadBMP(file):
    ...
    :rtype: `SDL_Surface`
    '''
    return SDL_LoadBMP_RW(rwops.SDL_RWFromFile(file, 'rb'), 1)

SDL_SaveBMP_RW = dll.function('SDL_SaveBMP_RW',
    ...
    arg_types=[POINTER(SDL_Surface), POINTER(rwops.SDL_RWops), c_int],
    return_type=c_int,
    success_return=0)
...
def SDL_SaveBMP(surface, file):
    ...
    :Parameters:
     ...
     - `dst`: `SDL_RWops`
    '''
    return SDL_SaveBMP_RW(surface, rwops.SDL_RWFromFile(file, 'wb'), 1)

SDL_SetColorKey = dll.function('SDL_SetColorKey',
...
SDL_SetAlpha = dll.function('SDL_SetAlpha',
...
SDL_SetClipRect = dll.function('SDL_SetClipRect',
...
_SDL_GetClipRect = dll.private_function('SDL_GetClipRect',
    arg_types=[POINTER(SDL_Surface), POINTER(SDL_Rect)],
    return_type=None)
...
SDL_ConvertSurface = dll.function('SDL_ConvertSurface',
...
SDL_UpperBlit = dll.function('SDL_UpperBlit',
...
SDL_BlitSurface = dll.function('SDL_UpperBlit',
    '''Perform a fast blit from the source surface to the destination
    surface.
...
SDL_LowerBlit = dll.function('SDL_LowerBlit',
    '''Low-level fast blit.
...
SDL_FillRect = dll.function('SDL_FillRect',
...
SDL_DisplayFormat = dll.function('SDL_DisplayFormat',
...
SDL_DisplayFormatAlpha = dll.function('SDL_DisplayFormatAlpha',
    '''Copy a surface to the pixel format and colors of the display
    buffer.
...
SDL_CreateYUVOverlay = dll.function('SDL_CreateYUVOverlay',
...
SDL_LockYUVOverlay = dll.function('SDL_LockYUVOverlay',
...
SDL_UnlockYUVOverlay = dll.function('SDL_UnlockYUVOverlay',
...
SDL_DisplayYUVOverlay = dll.function('SDL_DisplayYUVOverlay',
...
SDL_FreeYUVOverlay = dll.function('SDL_FreeYUVOverlay',
...
SDL_GL_SetAttribute = dll.function('SDL_GL_SetAttribute',
...
_SDL_GL_GetAttribute = dll.private_function('SDL_GL_GetAttribute',
...
SDL_GL_SwapBuffers = dll.function('SDL_GL_SwapBuffers',
...
_SDL_WM_SetCaption = dll.private_function('SDL_WM_SetCaption',
...
_SDL_WM_GetCaption = dll.private_function('SDL_WM_GetCaption',
...
_SDL_WM_SetIcon = dll.private_function('SDL_WM_SetIcon',
...
def SDL_WM_SetIcon(icon, mask):
    ...
    if mask:
        ref, mask = \
            array.to_ctypes(mask, (icon.w * icon.h + 7) / 8, c_ubyte)
    _SDL_WM_SetIcon(icon, mask)

SDL_WM_IconifyWindow = dll.function('SDL_WM_IconifyWindow',
    '''Iconify the window.
...
SDL_WM_ToggleFullScreen = dll.function('SDL_WM_ToggleFullScreen',
...
SDL_WM_GrabInput = dll.function('SDL_WM_GrabInput',
